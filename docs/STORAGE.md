...
Project gallery uploads accept PNG, JPEG, WebP, and passive SVG. SVG uses the same strict parser as Skill icons, so metadata such as `data-name` is allowed while DTD/entity declarations, scripts, event attributes, external references, and foreign namespaces are rejected. Each request contains one or more `files` parts plus a JSON `metadata` part whose zero-based `fileIndex` values map metadata to file order. The default request limit is 10 files (`Upload__MaxProjectGalleryFiles`, capped at 20), every image requires unique non-negative display order plus English and Vietnamese alt text, and generated keys use `projects/{projectId}/{uuid}.{extension}` in `SupabaseStorage__Buckets__ProjectImages`.
